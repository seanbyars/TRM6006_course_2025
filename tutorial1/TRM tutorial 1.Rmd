...
```{r GDC_workthrough_lastyear, eval=FALSE, include=FALSE}

# Have included this here for reference
# this was the manual GDC work through from last year, however the website/portal has changed - can no longer access the bcr biotab data
# rather it now downloads all datasets for the selected cohort

#### 2.1 setting up for a survival analysis - last year
- Repository
  - -> Data format - bcr biotab - 470 of 838 cases available, 16 files available

# nationwidechildrens.org_clinical_patient_skcm.txt # this is the file to download	
IN=read.table("~/Dropbox/2_CCS/5_teaching/2_project files 2 (DATED)/240326_MAIN_to_file/teaching/assignment 1 p1 code + walk through/nationwidechildrens.org_clinical_patient_skcm.txt",sep="\t",head=TRUE)
IN=IN[,c("bcr_patient_uuid","vital_status","last_contact_days_to","death_days_to","gender","height_cm_at_diagnosis","weight_kg_at_diagnosis","race")]
IN=IN[3:nrow(IN),] # remove first two rows
...
# Now need to add in the mutation status data
# Analysis Centre - mutation frequency - mutations - RAC1 P29S - Id's of individuals with this mutation obtained with the 'export cohort' function when cross-referencing with your chosen cohort
RAC1=read.table("~/Dropbox/2_CCS/5_teaching/2_project files 2 (DATED)/240326_MAIN_to_file/teaching/assignment 1 p1 code + walk through/RAC1.tsv",head=TRUE)
RAC1$RAC1m=1
names(RAC1)[1]="bcr_patient_uuid"

IN=merge(IN, RAC1,by="bcr_patient_uuid",all=TRUE)
IN$RAC1m[c(which(is.na(IN$RAC1m)))]=0

library(ggsurvfit)
#dev.new(height=4, width=4)
survfit2(Surv(time, status) ~ RAC1m, data = IN) %>% 
  ggsurvfit() +
  labs(
    x = "Days",
    y = "Overall survival probability"
...
survfit2(Surv(time, status) ~ as.factor(gender), data = IN) %>% 
  ggsurvfit() +
  labs(
    x = "Days",
    y = "Overall survival probability"
...
cox <- coxph(Surv(time, status) ~ RAC1m, data = IN); summary(cox)
cox <- coxph(Surv(time, status) ~ RAC1m + as.factor(gender), data = IN); summary(cox)
